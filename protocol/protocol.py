...
def radio_packet_parse(packet):
	...
	if packet[0] == b"H":
		...
		return {"_type": "heartbeat", "callsign": callsign, "id": id, "hear": hear}
	if packet[0] == b"D":
		_type,message, = struct.unpack('!c16s', packet)
		return {"_type": "debug", "message": message}
	if packet[0] == b"c":
		_type, = struct.unpack('!c', packet)
...
def radio_packet_pack(message):
	...
	if message["_type"] == "heartbeat":
		return struct.pack('!c12sBB', b"H", message["callsign"], message["id"], message["hear"])
	if message["_type"] == "debug":
		return struct.pack('!c16s', b"D", message["message"])
