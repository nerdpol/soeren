...
size_t ringbuf_put_many(struct ringbuf * rngbf, uint8_t const* buf, size_t count) {
	size_t original_count = count;
	if (rngbf->head <= rngbf->tail) { //write to the space between tail and end
		size_t writing = min(count, rngbf->buffer + sizeof(rngbf->buffer) - rngbf->tail);
...
uint8_t ringbuf_peek(struct ringbuf * rngbf) {
	return *rngbf->head;
}
